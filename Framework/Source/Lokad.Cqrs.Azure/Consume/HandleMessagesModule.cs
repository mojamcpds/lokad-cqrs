...
namespace Lokad.Cqrs.Consume.Build
{
    public sealed class HandleMessagesModule : Module
    {
        ...
        /// <returns>same module for inlining</returns>
        public HandleMessagesModule LogExceptionsToBlob(string containerName,
            params PrintMessageErrorDelegate[] delegates)
        {
        ...
        IEngineProcess ConfigureComponent(IComponentContext context)
        {
            ...
            log.DebugFormat("Use {0} threads to listen to {1}", NumberOfThreads,
                ExtendIEnumerable.Join(queueNames, "; "));
            return consumer;
        }
        ...
        void DebugPrintIfNeeded(ILog log, IMessageDirectory directory)
        {
            if (DebugPrintsMessageTree)
            {
                ...
                {
                    log.DebugFormat("{0} : {1}", info.MessageType.Name,
                        ExtendIEnumerable.Join(info.AllConsumers.Select(c => c.FullName), "; "));
                }
            }
            if (DebugPrintsConsumerTree)
            {
                ...
                {
                    log.DebugFormat("{0} : {1}", info.ConsumerType.FullName,
                        ExtendIEnumerable.Join(info.MessageTypes.Select(c => c.Name), "; "));
                }
            }
        }
