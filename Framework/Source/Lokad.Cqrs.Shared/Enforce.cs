...
namespace Lokad
{
    ...
    public static class Enforce
    {
        ...
        /// <remarks>Silverlight 2.0 does not support fast resolution of variable names, yet</remarks>
        [DebuggerNonUserCode]
        public static void Arguments<T1, T2, T3, T4, T5>(Func<T1> first, Func<T2> second, Func<T3> third,
            Func<T4> fourth,
            Func<T5> fifth)
            where T1 : class
            where T2 : class
            ...
            [InvokerParameterName] string paramName, [NotNull] string checkName)
        {
            if (!check)
            {
                throw new ArgumentException(
                    string.Format("Argument '{0}' has failed check '{1}'.", paramName, checkName), paramName);
            }
        }
        ...
        public static void That<T>(
            ...
            params Rule<T>[] rules)
        {
            var name = Reflect.VariableName(argumentReference);
        ...
        public static void That<T>(
            ...
            params Rule<T>[] rules)
        {
            var name = Reflect.VariableName(collectionReference);
            using (var scope = Scope.ForEnforce(name, Scope.WhenError))
            {
